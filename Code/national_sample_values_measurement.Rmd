...
create_lav_growth_model <- function(factor_name, manifest_vec, int.group_equal = F, var.group_equal = F, cov.group_equal = F, num_groups = 4, center_wave = 1){
  wave_factors <- paste0(factor_name, '_W', 1:4)
  intercept <- paste0(factor_name, '_i =~ ', 
                      paste(
                        paste0('1*', wave_factors),
                        collapse = ' + '))
  slope <- paste0(factor_name, '_s =~ ', 
                      paste(
