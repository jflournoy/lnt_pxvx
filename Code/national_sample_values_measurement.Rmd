...
create_lav_invariance_model <- function(factor_name, manifest_vec, type = c('unconstrained', 'long_metric', 'long_strong', 'long_strict', 'baseline','metric','strong', 'strict')){
  ...
  amod <- switch(type,
                 unconstrained = paste(c(create_lav_id_strat_widaman(factor_name),
                                   create_lav_factor_loadings(factor_name, manifest_vec, 
                                                              group_equal = F, long_free = T),
                                   create_lav_ints(manifest_vec, 
                                                   group_equal = F, long_free = T),
                                   create_lav_resid_var(manifest_vec, 
                                                        group_equal = F, long_free = T),
                                   create_lav_resid_covar(manifest_vec, 
                                                          group_equal = F, long_free = T)), 
                                 collapse = '\n\n#---\n\n'),
                 long_metric = paste(c(create_lav_id_strat_widaman(factor_name),
                                   create_lav_factor_loadings(factor_name, manifest_vec, 
                                                              group_equal = F, long_free = F),
                                   create_lav_ints(manifest_vec, 
                                                   group_equal = F, long_free = T),
                                   create_lav_resid_var(manifest_vec, 
                                                        group_equal = F, long_free = T),
                                   create_lav_resid_covar(manifest_vec, 
                                                          group_equal = F, long_free = T)), 
                                 collapse = '\n\n#---\n\n'), 
                 long_strong = paste(c(create_lav_id_strat_widaman(factor_name),
                                   create_lav_factor_loadings(factor_name, manifest_vec, 
                                                              group_equal = F, long_free = F),
                                   create_lav_ints(manifest_vec, 
                                                   group_equal = F, long_free = F),
                                   ...
                                   create_lav_resid_covar(manifest_vec, 
                                                          group_equal = F, long_free = T)), 
                                 collapse = '\n\n#---\n\n'),
                 long_strict = paste(c(create_lav_id_strat_widaman(factor_name),
                                   create_lav_factor_loadings(factor_name, manifest_vec, 
                                                              group_equal = F, long_free = F),
                                   create_lav_ints(manifest_vec, 
                                                   group_equal = F, long_free = F),
