...
#temp
manifest_vec <- get_subscale_items(lnt_scales_df, 'bfi_e')
group_equal <- F
cat(create_lav_ints(get_subscale_items(lnt_scales_df, 'bfi_a'), group_equal = F))
item <- manifest_vec[1]
#temp
...
create_lav_resid_covar <- function(item, waves = 4){
  wave_items <- paste0(letters[1:waves], item)
...
cat(create_lav_factor_loadings('bfi_e', get_subscale_items(lnt_scales_df, 'bfi_e')))
...
check_manifests_exist(get_subscale_items(lnt_scales_df, 'bfi_e'), lnt_nat_decades_items)
bfi_e_model <- create_lnt_unifactor_model('bfi_e', get_subscale_items(lnt_scales_df, 'bfi_e'))
bfi_e_varlist <- create_longitudinal_var_list(get_subscale_items(lnt_scales_df, 'bfi_e'))
