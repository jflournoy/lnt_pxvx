...
wave1_scored <- psych::scoreItems(keys = wave1_keys[-25], lnt_items_nat, impute = 'none',
                                  min = 1, max = 5)
wave1_scored_mvi <- psych::scoreItems(keys = wave1_keys[25], lnt_items_nat, impute = 'none',
                                      min = 1, max = 9)
wave2_keys <- create_wave_keys(lnt_scales_df, 'b')
wave2_scored <- psych::scoreItems(keys = wave2_keys[-25], lnt_items_nat, impute = 'none',
                                  min = 1, max = 5)
wave2_scored_mvi <- psych::scoreItems(keys = wave2_keys[25], lnt_items_nat, impute = 'none',
                                      min = 1, max = 9)
wave3_keys <- create_wave_keys(lnt_scales_df, 'c')
wave3_scored <- psych::scoreItems(keys = wave3_keys[-25], lnt_items_nat, impute = 'none',
                                  min = 1, max = 5)
wave3_scored_mvi <- psych::scoreItems(keys = wave3_keys[25], lnt_items_nat, impute = 'none',
                                      min = 1, max = 9)
wave4_keys <- create_wave_keys(lnt_scales_df, 'd')
wave4_scored <- psych::scoreItems(keys = wave4_keys[-25], lnt_items_nat,  impute = 'none',
                                  min = 1, max = 5)
wave4_scored_mvi <- psych::scoreItems(keys = wave4_keys[25], lnt_items_nat, impute = 'none',
                                      min = 1, max = 9)

all_waves_scored <- bind_rows(
  list(`1` = bind_cols(as_data_frame(wave1_scored$scores), as_data_frame(wave1_scored_mvi$scores)),
       `2` = bind_cols(as_data_frame(wave2_scored$scores), as_data_frame(wave2_scored_mvi$scores)),
       `3` = bind_cols(as_data_frame(wave3_scored$scores), as_data_frame(wave3_scored_mvi$scores)),
       `4` = bind_cols(as_data_frame(wave4_scored$scores), as_data_frame(wave4_scored_mvi$scores))),
  .id = 'wave')
...
# drop_items <- list(
#   'aspfin' = c('asp14'), #most big MI, and lowest loading
#   'bfa_mt' = c('bfas113'), #most MI, lowest loading
#   'hrz_col' = c('ind9'), #biggest MIs, but high loading
#   'hrz_ind' = c('ind4'), #not much problematic here, but this has lowest loading
#   'usi' = c('usi1'), #most evidence from MI, but usi5 and usi2 are very skewed, might be worth checking.
#   'vrt_col' = c('ind14'), #mi evidence, esp. loadings and variance. very skewed too.
#   'vrt_ind' = c('ind8'), #some mi, lowest loading, "tense and aroused"
# )
...
#-.007 and -.01 
conclusions_list[['Financial Aspirations']] <- paste0('Some improvement. Better overall fit, improved invariance, but still has $\\Delta\\text{MFI}$ = .016 (versus .023 previously). Dropped ', lnt_item_labels_to_text(drop_items[['aspfin']]))
conclusions_list[['Materialism']] <- paste0('Some improvement. Better overall fit, improved invariance, but still has $\\Delta\\text{MFI}$ = .016 (versus >.024 previously). Also solved convergence problems. Dropped  ', lnt_item_labels_to_text(drop_items[['bfa_mt']]))
conclusions_list[['Horizontal Collectivism']] <- paste0('Some improvement. Better overall fit, improved invariance, but still has $\\Delta\\text{MFI}$ = .032 (versus .056 previously). Dropped ', lnt_item_labels_to_text(drop_items[['hrz_col']]))
conclusions_list[['Horizontal Individualism']] <- paste0('About the same. Better overall fit. May not be necessary to drop items (but $\\Delta\\text{MFI}$ is .12 from long strict to strict in both cases). Dropped ', lnt_item_labels_to_text(drop_items[['hrz_ind']]))
conclusions_list[['Mature Values Index']] <- ''
conclusions_list[['Unmitigated Self-Interest']] <- paste0('Worse invariance, but better overall fit indices. Dropped  ', lnt_item_labels_to_text(drop_items[['usi']]))
conclusions_list[['Vertical Collectivism']] <- paste0('Some improvement. Still not perfect. Only three items. Very good fit indices. Dropped ', lnt_item_labels_to_text(drop_items[['vrt_col']]))
conclusions_list[['Vertical Individualism']] <- paste0('Better. Difference between strict and long_strict is ~< .01. Dropped ', lnt_item_labels_to_text(drop_items[['vrt_ind']]))
conclusions_list[['Social Self-Regulation']] <- ''
conclusions_list[['.Conscientiousness']] <- ''
conclusions_list[['..Industriousness']] <- ''
conclusions_list[['..Orderliness']] <- ''
