...
create_lav_factor_loadings <- function(factor_name, manifest_vec, group_equal = F){
  ...
  load_labels <- lapply(paste0('L', 2:num_indicators),
                        function(label){
  ...
  loadings <- c('c(1, 1, 1, 1)*', paste0(load_labels, '*'))
...
create_lav_resid_covar <- function(manifest_vec, group_equal = F){
  covar_grp_eqs <- lapply(manifest_vec, function(man_var){
    var_label <- paste0('cv_', which(manifest_vec == man_var))
    grp_labels <- rep(var_label, 4)
    if(!group_equal){
      grp_labels <- paste0(grp_labels, 'g', 1:4)
    }
    grp_labels_collapsed <- paste(grp_labels, collapse = ', ')
    connector <- paste0(' ~~ c(', grp_labels_collapsed, ')*')
    wave_vars <- paste0(letters[1:4], man_var)
    covar_label_matrix <- combn(wave_vars, 2)
    eqs <- apply(covar_label_matrix, 2, paste, collapse = connector)
...
bfi_e_model <- paste(c(create_lav_factor_loadings('bfi_e', these_items),
                       create_lav_ints(these_items),
                       create_lav_resid_covar(these_items),
                       create_lav_resid_var(these_items)), 
                     collapse = '\n\n#---\n\n')

m1 <- sem(bfi_e_model, data = lnt_nat_decades_items, missing = 'fiml', group = 'decade_group')
summary(m1, standardized = T)

bfi_e_model.metric <- paste(c(create_lav_factor_loadings('bfi_e', these_items, group_equal = T),
                              create_lav_ints(these_items),
                              create_lav_resid_covar(these_items),
                              create_lav_resid_var(these_items)), 
                            collapse = '\n\n#---\n\n')

m2 <- sem(bfi_e_model.metric, data = lnt_nat_decades_items, missing = 'fiml', group = 'decade_group')
summary(m2, standardized = T)

lavaan::anova(m1, m2)

bfi_e_model.strong <- paste(c(create_lav_factor_loadings('bfi_e', these_items, group_equal = T),
                              create_lav_ints(these_items, group_equal = T),
                              create_lav_resid_covar(these_items),
                              create_lav_resid_var(these_items)), 
                            collapse = '\n\n#---\n\n')

m3 <- sem(bfi_e_model.strong, data = lnt_nat_decades_items, missing = 'fiml', group = 'decade_group')
summary(m3, standardized = T)

(anova_thing <- anova(m1, m2, m3))
(fit_list <- rbind(m1=fitmeasures(m1, fit.measures = c('cfi', 'rmsea')), 
                  m2=fitmeasures(m2, fit.measures = c('cfi', 'rmsea')), 
                  m3=fitmeasures(m3, fit.measures = c('cfi', 'rmsea'))))
