...
run_invariance_tests <- function(factor_name, manifest_vec, item_data, fit_measures, group, return_models = F, ...){
  ...
  lrt_test <- do.call(
    function(...){ 
      lavTestLRT(..., model.names = all_invar_types)
    }, 
    invar_models_list)
...
test_factors_for_invariance <- function(factor_names, indicators_df, item_data, fit_measures = c('cfi', 'rmsea', 'rmsea.ci.lower', 'rmsea.ci.upper'), group, ...){
  require(future)
...
factor_names <- unique(lnt_scales_df$factor_name)
fit_measures <- c('cfi', 'rmsea', 'rmsea.ci.lower', 'rmsea.ci.upper')

invar_test_rez_fn <- '../Rez/invariance_measures.Rds'
...
invar_tests_and_models_df_w <- mutate_at(
  ...
  vars(dAIC = AIC, dBIC = BIC, dCFI = cfi),
  function(x) x - lag(x))
invar_tests_and_models_df_w <- mutate_at(
  group_by(invar_tests_and_models_df_w, factor_name),
  vars(d2AIC = AIC, d2BIC = BIC, d2CFI = cfi),
...
nothingburger <- do(group_by(invar_tests_and_models_df_w, factor_name),{
  ...
  comm <- paste0('\\hline \n ',
                 paste(paste0('\\multicolumn{12}{l}{', 
                       ...
                       collapse = '\n'), '\n')
  print(xtable::xtable(table_df[, -c(6,9)], caption = title, align = c('r','l',rep('r', 10))), 
