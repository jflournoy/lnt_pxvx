...
#+ setup chunk,echo=FALSE, results='hide'
...
# createModels('workAndC_BiTemplate_base_ladders_bfas_Gender.inp')
...
biParams<-ldply(biModelOut,
                function(aMod){
                  filenameSub<-function(x){
                    sub('-((nat)|(col)|(nat empl))-base(-(male)|-(female))*\\.out','',x)
                  }
                  subsetSub<-function(x){
                    sub('.*((nat)|(col)|(nat empl)).*','\\1',x)
                  }
                  genderSub<-function(x){
                    sub('.*-(\\w+)\\.out','\\1',x)
                  }
                  if(!exists('Parameters',where=aMod$summaries)){
                    data.frame(
                      filename=filenameSub(aMod$summaries['Filename']),
                      subset=subsetSub(aMod$summaries['Filename']),
                      gender=genderSub(aMod$summaries['Filename']),
                      param_l=NA,
                      param_l_wave=NA,
                      relat=NA,
                      param_r=NA,
                    ...
                    leftPattern<-'(\\w)_*(BFI_\\w{1,2}|BFAS_\\w{1,2}|ASPFIN|GOALEC|INVJOB|LADDE1|LADDE2|ECOFIN)\\.(\\w+)'
                    rightPattern<-'(\\w)_*(BFI_\\w{1,2}|BFAS_\\w{1,2}|ASPFIN|GOALEC|INVJOB|LADDE1|LADDE2|ECOFIN)'
                    parDF$param_l<-
                      sub(
                        leftPattern,
                        '\\2',
                    ...
                    aDF$filename<-filenameSub(aMod$summaries['Filename'])
                    aDF$subset<-subsetSub(aMod$summaries['Filename'])
                    aDF$gender<-genderSub(aMod$summaries['Filename'])
                    aDF<-aDF[,c('filename',
                                'subset',
                                'gender',
                                'param_l',
...
#+ display chunk,results='asis',warning=TRUE
d_ply(biParams,
      .variables=c('subset'),
      function(subsetDF){
        ...
        d_ply(biParams[biParams$subset==aSubset,],
              .variables=c('filename','gender'),
              function(aDF){
                biScaleNames<-sub('_',' ',paste(strsplit(aDF$filename[1],'-')[[1]][2:3],collapse=' '))
                write(paste('\n## Scales:',sub('_',' ',paste(strsplit(aDF$filename[1],'-')[[1]][2:3],collapse=', ')),aDF$gender[1],'\n\n'),'')
                notScales<-c('Means','Intercepts','Variances','Residual.Variances')
